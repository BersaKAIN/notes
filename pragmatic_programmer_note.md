...
A good method should do no more or less than it claims to do. But how do we determine what a method claims? A common approach will be DBC.

- Caller of the method should make sure that the pre-condition of the method is matched.
- The author of the method should make sure once the pre-condition is met then after the execution of the method, the post-condition is also met and the method invariant is valid.
- Subclass can only make pre-condition weaker and post-condition stronger. All its parent claims to do should still be done.

** Assertions

All of us programmers have once thought, this can never happen. If you really think so, check it.

** Exceptions and Error Handling

Exceptions should be used in trully exceptional unexpected situations. If you are reading an file that is supposed to be present on all machines, then an exceptions is more approriate. However, when you are reading a user specified file, then maybe an error handler is better.
...
*** Minimizing Coupling
The less we know about other packages, the less dependant they are to each other. And this is good. Coupling will cause a change in one place affect many other places. It will also make programmer less confident about their program and dare not to make changes in the program because they don't even know what would happen.
